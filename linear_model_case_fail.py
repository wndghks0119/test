...
for epoch in range(1,epoches+1):
    ...
    # 방법 3) L1 경사하강법 적용하기
    change_function = np.ones((m+1,1))
    theta_method3 = 0.5*np.ones((m+1,1))
    for iteration in range(1,total_iteration+1):
        for parameter in range(0,m+1):
            # 편미분 작업을 편하게 하기 위한 전처리
            X_batch_para = X_batch_big[:,parameter].reshape(batch_size,1)
            # 각 파라미터 별로 편미분 구하기
            change_function[parameter] = np.sum(2 * (np.dot(X_batch_big,theta_method3)-y_batch) * theta_method3[parameter,0] * X_batch_para, dtype=np.int64)
            # 구한 편미분을 통해 파라미터 업데이트
            theta_method3[parameter] = theta_method3[parameter] - alpha * change_function[parameter]    
    
    print(f'L1 경사하강법 사용 ) 절편 : {theta_method3[0]}, 파라미터 : {theta_method3[1:]}')
    ...
    for iteration in range(1,total_iteration+1):
        for parameter in range(0,m+1):
            ...
            X_batch_para = X_batch_big[:,parameter].reshape(batch_size,1)
            # 각 파라미터 별로 편미분 구하기
            change_function[parameter] = np.sum(2 * (np.dot(X_batch_big,theta_method4)-y_batch) * theta_method4[parameter,0] * X_batch_para, dtype=np.int64)
            # 구한 편미분을 통해 파라미터 업데이트
            theta_method4[parameter] = theta_method4[parameter] - alpha * change_function[parameter]    
    
    print(f'L2 경사하강법 사용 ) 절편 : {theta_method4[0]}, 파라미터 : {theta_method4[1:]}')
